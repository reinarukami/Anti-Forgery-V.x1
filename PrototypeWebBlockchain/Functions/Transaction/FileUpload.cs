...
namespace PrototypeWebBlockchain.Repository
{
    public class FileUpload : DefaultWeb3
    {
        ...
        public void UploadFile(FileClass _file, TransactionRepository _transactionRepository, string _memberID)
        {
            ...
            var TaskCheckGas = FileContract.GetFunction("AddFiles").EstimateGasAsync(HttpContext.Current.Session["Address"].ToString(), null, null, imageId, _file.hash.ToString(), DateTime.Now.ToString());
            TaskCheckGas.Wait();

            var TaskAddFile = FileContract.GetFunction("AddFiles").SendTransactionAsync(HttpContext.Current.Session["Address"].ToString(), imageId, _file.hash.ToString(), DateTime.Now.ToString());
            TaskAddFile.Wait();
        ...
        public void GetFiles()
        {
            ...
            var TaskGetFile = FileContract.GetFunction("GetFiles").CallDeserializingToObjectAsync<FileDTO>(0);
            TaskGetFile.Wait();
